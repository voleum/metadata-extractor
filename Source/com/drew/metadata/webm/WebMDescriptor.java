...
import static com.drew.metadata.webm.WebMDirectory.*;

public class WebMDescriptor<T extends Directory> extends TagDescriptor<WebMDirectory> {

    public WebMDescriptor(@NotNull WebMDirectory directory)
    {
        super(directory);
    }
    ...
    {
        byte[] value = _directory.getByteArray(WebMDirectory.TAG_MAJOR_BRAND);
        if (value == null)
            return null;
    ...
    {
        String[] values = _directory.getStringArray(WebMDirectory.TAG_COMPATIBLE_BRANDS);
        if (values == null)
            return null;
